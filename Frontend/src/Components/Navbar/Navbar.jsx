  ...
  return (
    <nav className="navbar">
      ...
      <ul className = {Sdisplay} ref={Navref}>
        ...
        {!isAuthenticated ? (
          <li className = "logger" onClick={() => loginWithRedirect()}>Log In</li>
        ) : (
          <>
            <li className="user-info">
              <img className="user-image" src={user?.picture} alt="User Profile" width={30} /> 
              <span className="user-name">{user?.name}</span>
            </li>
            <li className="logger" onClick={() => logout({ returnTo: window.location.origin })}>Log Out</li>
          </>
        )}
      ...
      <ul className="sidebar">
        ...
        {!isAuthenticated ? (
          ...
          <>
            <li>
              <img className = "user-image" src={usericon} alt="User Image" />
            </li>
            <li className = "logger" onClick={() => logout({ returnTo: window.location.origin })}>Log Out</li>
          </>
        )}
